...
class Pessoa(models.Model):
    ...
    POSSIVEL MELHORIA FUTURA: Considerar a adição de um campo 'tipo_pessoa' (CharField
    com choices PF/PJ) diretamente nesta tabela. Isso eliminaria a necessidade
    de consultas extras (N+1) ao banco para descobrir se uma Pessoa é Física
    ou Jurídica via try/except nos Serializers e no Admin, bastando ler o
    campo direto que já veio na mesma query. A alteração exigiria uma nova
    migração e ajustes nos services para preencher o campo na criação.
